...
exports.welcomeQuestion = exports.CreateQuestion = void 0;
var _option = require("../../database/option");
var _question = require("../../database/question");
// trial
const welcomeQuestion = (req, res) => {
  try {
 ...
 *  Creating Question
 */
exports.welcomeQuestion = welcomeQuestion;
const CreateQuestion = async (req, res) => {
  try {
    ...
    // creating questions
    const newQuestion = await _question.QuestionModel.create({
    ...
    const createdOptions = await _option.OptionModel.insertMany(options.map(optionText => ({
      questionId: newQuestion._id,
      option: optionText
    })));

    // Set correct option for the question
    const correctOptionDoc = createdOptions.find(option => option.option === correctOption);
    if (correctOptionDoc) {
      await _question.QuestionModel.findByIdAndUpdate(newQuestion._id, {
      ...
      const populatedQuestion = await _question.QuestionModel.findById(newQuestion._id).populate("options", "option") // Populating options with only 'option' field
      ...
      return res.status(400).json({
        error: "Correct option not found among the provided options"
      });
    }
  } catch (error) {
    console.error(error);
